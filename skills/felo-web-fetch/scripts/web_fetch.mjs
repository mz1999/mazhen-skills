...
function usage() {
  console.error(
    [
      'Usage:',
      '  node web_fetch.mjs --url <url> [options]',
      '',
      'Required:',
...
function isApiError(payload) {
  ...
  if (typeof payload?.status === 'string') return payload.status.toLowerCase() === 'error';
  return false;
}
...
async function fetchJson(url, init, timeoutMs) {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`HTTP ${res.status}: ${getErrorMessage(body)}`);
    }
    if (isApiError(body)) {
...
function buildPayload(args) {
  ...
  if (args.cookiesJson) {
    try {
      ...
      if (!Array.isArray(parsed)) throw new Error('set_cookies JSON must be an array');
      payload.set_cookies = parsed;
    } catch (err) {
      throw new Error(`Invalid --set-cookies-json: ${err.message}`);
    }
  }
